...
pro ds9_reg_maker, reg_file, el, as_pix
    ...
    if file_exist(reg_file) then begin
            
      ; read ds9 region file, and then removing it   
      ...
      for i=0, n_elements(array)-1 do begin
         ; ignore elliptical appertures, to be replaced later in this routine
         if  strcmp(array[i], 'ellipse', 7, /FOLD_CASE) NE 1 then printf, lun, array[i]
      endfor            
    
    endif else begin  ; if ds9 region file does not exist 
...
for i=0, n_elements(array)-1 do begin
 ...
 if strcmp(array[i], 'circle', 6, /FOLD_CASE) EQ 1 and $ 
     strmatch(array[i], '*NoColor*', /FOLD_CASE) NE 1 then begin  ; '*red*'
    line =  array[i]
...
if n_elements(circles) gt 1 then circles = circles[1:n_elements(circles)-1]
...
if n_elements(box) gt 1 then  box = box[1:n_elements(box)-1]
...
COMMON stvb, im, tim, sbase, draw, smin, smax, slog, sasinh, strue, ssmooth, $
	     ...
	     id, imfil, jpg_base, aux_base, anulus_col, tv_filter, edit_region, has_counter, last_command, reg_size, lock_reg, ellipse_glga, nlist
...
PRO stv_find,galaxy=galaxy,outside=outside,noprompt=noprompt, plus=plus, minus=minus

COMMON stvb, im, tim, sbase, draw, smin, smax, slog, sasinh, strue, ssmooth, $
	     have_astr, as_pix, astr, dtype, rfile, browse, qa, wsrc, plog, $
             keyboard_text_id, hdr, cfile, nsrc, psrc, maxp, el, elorig, misc, $
	     id, imfil, jpg_base, aux_base, anulus_col, tv_filter, edit_region, has_counter, last_command, reg_size, lock_reg, ellipse_glga, nlist
common sigmult, maglim

;
p = ''
msg_txt = 'Are you sure, finding new point sources?'
digalog_warning, msg_txt
p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, TITLE='New point sources?')
print, 'The TV got activated :)'
...
if keyword_set(noprompt) then begin
	...
	if not keyword_set(plus) and not keyword_set(minus) then begin
	   sigmult=defmult
	   maglim=defmagl
	endif else begin
	   
	   if keyword_set(plus) then maglim+=0.1
	   if keyword_set(minus) then maglim-=0.1
	   sigmult=defmult
	endelse
...
if not tag_exist(ev,'type') then begin 
     ...
     if strcmp(strmid(UVALUE,strpos(UVALUE,'profile')), 'profile') then begin
         
        filt = strmid(UVALUE,0,strpos(UVALUE,'profile')-1)
	single_phot = strmid(aux_base,0,strpos(aux_base,'aux'))+'/plots/'+id+'_'+filt+'.jpg'
	...
	if finfo.exists then begin
		...
		tvscl, jplot4, /true
	        ...
	        if not strcmp(filt,'multiband') and not strcmp(filt,'images') then $ 
	           print, ':( The corresponding file does not exist, pleae run the photometry'
	        
	endelse
	
        if strcmp(filt,'multiband') then begin
            
            photplot = strmid(aux_base,0,strpos(aux_base,'aux'))+'/plots/'+id+'_'+dtype+'_profile.jpg'
            ...
            endif else print, ':( The corresponding file does not exist, pleae run the photometry'                
                  
        endif   
        
        
        if strcmp(filt,'images') then begin
            
            imageplot = strmid(aux_base,0,strpos(aux_base,'aux'))+'/plots/'+id+'_'+dtype+'_images.jpg'
     ...
     endif  ; END Profile on the Menu-bar
     ...
     if strcmp(UVALUE, 'clear_sex') then begin
        msg_txt = 'Remove all SExtractgor masks?'
        digalog_warning, msg_txt
        q = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO)
        print, 'The TV got activated :)'
     ...
     if strcmp(UVALUE, 'set_back_radius') then Command_handler, '}'
     if strcmp(UVALUE, 'get_back_radius') then Command_handler, '{'
     if strcmp(UVALUE, 'reset_back_radius') then Command_handler, ':'
     ...
     if strcmp(UVALUE, 'goto') then  Command_handler, 'goto'
...
;               XYOUTS, ev.x, ev.y, 'O', /DEVICE
              ...
              msg_txt = 'Do you really want to re-center the ellispe?'   
              digalog_warning, msg_txt
              p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO)
              print, 'The TV got activated :)'
		...
		if n_elements(Roi) gt 1 then  Roi = Roi[1:n_elements(Roi)-1]
...
Pro switch_image, band, sdss_only=sdss_only

COMMON stvb, im, tim, sbase, draw, smin, smax, slog, sasinh, strue, ssmooth,$
	     have_astr, as_pix, astr, dtype, rfile, browse, qa, wsrc, plog, $
             keyboard_text_id, hdr, cfile, nsrc, psrc, maxp, el, elorig, misc, $
	     id, imfil, jpg_base, aux_base, anulus_col, tv_filter, edit_region, $
	     has_counter, last_command, reg_size, lock_reg, ellipse_glga, nlist
...
if not keyword_set(sdss_only) then begin

        if strpos(dtype,'sdss') ge 0  or strpos(dtype,'panstarrs') ge 0  then begin
          if not file_exist(jpg_base+'_'+band+'.jpg') then begin 
	        print, 'Warning: Could NOT find: ' + jpg_base+'_'+band+'.jpg'
	        if file_exist(rute+'_'+band+'.fits*') then begin
	           fspec  = rute+'_'+band+'.fits*'
	           outdir = strmid(jpg_base,0,strpos(jpg_base,id))
	           digalog_warning, 'The JPG file not found !'
	           q = DIALOG_MESSAGE('Could not find the jpg file, but the fits file may be available !',  /INFORMATION,  /CENTER, TITLE='Missing JPG file !')
                   p = DIALOG_MESSAGE('Do you want to generate this jpg file?', /QUESTION,  /CENTER, TITLE='Regenerate JPG file?')
                   print, 'The TV got activated :)'
                   if  strcmp(p, 'Yes') then begin
                       if strcmp(band, 'u') then begin
	                  if strpos(dtype,'sdss') ge 0 then rgb_sdss_asinh,fspec=fspec,outdir=outdir,update=update,verbose=verbose,/uonly  
               endif
                       if strcmp(band, 'g') then begin
          ...
          if file_exist(jpg_base+'_'+band+'.jpg') then begin 
                jpg_file = jpg_base+'_'+band+'.jpg'
...
case eventchar of
    ...
    'star+': if browse eq 0 then $
		    stv_find, /noprompt, /plus
    'star-': if browse eq 0 then $
		    stv_find, /noprompt, /minus
    'starf': if browse eq 0 then $
		    stv_find, /noprompt
    ...
    'i': invert_colortable
    '}': modify_background
    '{': show_modify_background
    ':': reset_modify_background
    'k': begin
	 for i=0l,maxp-1l do $
    ...
    'm': if browse eq 0 then begin
            runme = 1
            if qa.backradius gt 0 then begin
               msg_txt = 'Background modification radius is NOT zero: '+strtrim(string(qa.backradius, FORMAT='(F5.2)'),2)+' (arcmin) - Continue?'
               p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, TITLE='Continue?')
               if not strcmp(p, 'Yes') then runme = 0
            endif
            
            if runme eq 1 then begin
	 	misc = 'RUN'
	 	stv_shutdown, status='tmp'
	     endif
    ...
    'x': begin
         ...
         msg_txt = 'Do you really want to SKIP?'
         digalog_warning, msg_txt
         p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO)
         print, 'The TV got activated :)'
         if strcmp(p, 'Yes') then begin
    ...
    'previous': begin
         ...
         msg_txt = 'Do you really want to go back?'
         digalog_warning, msg_txt
         p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO)
         print, 'The TV got activated :)'
         if strcmp(p, 'Yes') then begin
    ...
    'goto': begin
         ...
         msg_txt = 'Enter the galaxy row number in the Terminal:'
         digalog_warning, msg_txt
         p = DIALOG_MESSAGE(msg_txt, /Information,  /CENTER)
         print, 'The TV got activated :)'
         nloop = nlist[1]
         
         prompt='Which galaxy (1-'+strtrim(string(nloop),2)+'): '
  	 imagl=''
  	 read,imagl,prompt=prompt
  	 
  	 imagl = uint(abs(imagl))
  	 iloop = imagl
             if imagl le nloop and imagl ge 1 then begin
                     nlist[0] = iloop-2L
                     misc = 'GOTO'
                     stv_shutdown, status='tmp'
             endif else print, 'Wrong value :('
	 end      
...
if strcmp(eventchar, 'import_reg') then begin
   ...
   s=size(im)
   openw, lun, edit_region, /append, /get_lun
   printf,lun,'# DS9 ROI output (image index): '+systime(0)
   printf,lun,'# IMAGE NX,NY: ',s[1],s[2]
   printf,lun, 0
   free_lun,lun
...
if strcmp(eventchar, '\') or strcmp(eventchar, '9') then begin
      ...
      if not has_counter then begin; contour is NOT activated
          ...
          if strcmp(dtype,'sdss') then begin
            spawn, "xpaset -p ds9 contour nlevels 37"
            spawn, "xpaset -p ds9 contour smooth 6"
          endif else begin
            spawn, "xpaset -p ds9 contour nlevels 23"
            spawn, "xpaset -p ds9 contour smooth 4"
          endelse
...
if strcmp(eventchar, 'c_ds9') and strcmp(results[0], 'yes') then begin
  msg_txt = 'Do you really want to close all ds9 windows?'
  digalog_warning, msg_txt
  p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO)  
  print, 'The TV got activated :)'
...
;--------------------------------------------------------------------
Pro digalog_warning, DIALOG_TXT
  
  print, ''
  print, '***************************'
  print, 'The TV is frozen ... !!!'
  print, 'See the Dialoge Message !'
  print, DIALOG_TXT
  print, '***************************'
  print, ''
...
function get_roi_sep_idl7,im,rfile
...
COMMON stvb, im, tim, sbase, draw, smin, smax, slog, sasinh, strue, ssmooth,$
         ...
         msg_txt = 'Enter the back-radius in the Terminal (arcmin):'
         digalog_warning, msg_txt
         p = DIALOG_MESSAGE(msg_txt, /Information,  /CENTER)
         print, 'The TV got activated :)'

         print, 'Current Background modification Radius: '+ strtrim(string(qa.backradius, FORMAT='(F5.2)'),2)+' (arcmin)'
         prompt='back-radius (arcmin): '
  	 radius=''
  	 read,radius,prompt=prompt
  	 
  	 backrad = float(abs(radius))
  	 
  	 if backrad gt 0.0 then begin 
  	    qa.backradius = backrad
  	    print, 'Set Background modification Radius: '+ strtrim(string(qa.backradius, FORMAT='(F5.2)'),2)+' (arcmin)'
  	    print, 'Continue your work ... :)'
  	    print
  	 endif else begin 
  	    print
  	    print, 'Previous Background modification Radius: '+ strtrim(string(qa.backradius, FORMAT='(F5.2)'),2)+' (arcmin)'
  	    print, 'RESET (No Manual Background Modification)'
  	    qa.backradius = 0.
  	    print, 'Current Background modification Radius: '+ strtrim(string(qa.backradius, FORMAT='(F5.2)'),2)+' (arcmin)'
  	    print, 'Continue your work ... :)'
  	    print
...
case eventchar of
    ...
    'c' : begin
		...
		'stv_keyboard_event'
		 ...
		 WIDGET_CONTROL, WIDGET_INFO(pnale_w, FIND_BY_UNAME='colBase31_widg'), sensitive=1
...
if (xregistered ('flag_base')) then begin  
  ...
  group = WIDGET_INFO(flag_w, FIND_BY_UNAME='flag_gr2_uname')
  WIDGET_CONTROL, group, SET_VALUE=[qa.disturbed,qa.trail,qa.not_spiral,qa.face_on]
    
  group = WIDGET_INFO(flag_w, FIND_BY_UNAME='flag_gr3_uname')
  WIDGET_CONTROL, group, SET_VALUE=[qa.faint,qa.crowded,qa.over_masked]  
...
if cancel then begin
  msg_txt = 'Are you sure, cancel changes? (Yes: Cancel / NO: Save Changes)'
  p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO, TITLE='cancel?')
  print, 'The TV got activated :)'
...
q=''
msg_txt = 'Delete point sources in region?'
digalog_warning, msg_txt
q = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER)
print, 'The TV got activated :)'
if strcmp(q, 'Yes') then begin
...
if  strcmp(q, 'No') then begin
    p = ''
    msg_txt = 'Are you sure, you want to remove all masks?'
    digalog_warning, msg_txt
    p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO)
    print, 'The TV got activated :)'
    if  strcmp(p, 'Yes') then begin
...
q=''
msg_txt = 'Remove background region?'
digalog_warning, msg_txt
q = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO)
print, 'The TV got activated :)'
...
q=''
msg_txt = 'Keep '+region_name+' region?'
digalog_warning, msg_txt
q = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER)
print, 'The TV got activated :)'
if strcmp(q, 'Yes') then begin
...
if strcmp(strtrim(tv_filter,2), 'w_com') or strcmp(strtrim(tv_filter,2), 'n_com') then begin 
  ...
  msg_txt = "Note: Using '"+ imfil_band +"' band ... !"
  digalog_warning, msg_txt
  q = DIALOG_MESSAGE(msg_txt,  /INFORMATION,  /CENTER)
  print, 'The TV got activated :)'
...
if file_exist(sextract_file) then begin
   ...
   msg_txt = 'Use existing SExtract info?'
   digalog_warning, msg_txt
   p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, TITLE='Use exisiting file?')
   print, 'The TV got activated :)'
   if  strcmp(p, 'Yes') then begin
   ...
   print, 'flag: ', results
      ...
      if n_elements(results) ne 5 then begin
            msg_txt = 'Warning, something went wrong, Want to re-run SExtractor?'
            digalog_warning, msg_txt
	    p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER)
	    print, 'The TV got activated :)'
...
if not int_fi.exists then begin 
    print, intfile+': Cannot find the input file.'
    goto, finish
endif
...
; ########### SPAWN SEXTRACTOR
fits_org = file_exist(rute + '/'+id+'_'+filter+'.fits')
fits_gz  = file_exist(rute + '/'+id+'_'+filter+'.fits.gz')
if not fits_org and fits_gz then begin
spawn, 'cp '+ rute + '/'+id+'_'+filter+'.fits.gz '+ rute + '/'+id+'_'+filter+'.fits2.gz '
spawn, 'gunzip '+ rute + '/'+id+'_'+filter+'.fits2.gz '
spawn, 'mv '+ rute + '/'+id+'_'+filter+'.fits2 '+ rute + '/'+id+'_'+filter+'.fits '
endif
spawn, scr_command, results
; If a decompressed file has been generated, remove it to save some memory 
if not fits_org and fits_gz then spawn, 'rm '+ rute + '/'+id+'_'+filter+'.fits '
; ########### \END SPAWN SEXTRACTOR
...
if n_elements(results) ne 5 then begin
  ...
  print, ' '
  msg_txt = 'Error: Something went wrong ...'
  digalog_warning, msg_txt
  q = DIALOG_MESSAGE(msg_txt,  /INFORMATION,  /CENTER, TITLE='choose the center')
  print, 'The TV got activated :)'
...
if n_elements(results) eq 5 then begin
 ...
 if el_2 eq 0 or el_3 eq 0 or el_0 eq 0 or el_1 eq 0 then begin 
  print, ' '
  msg_txt = 'Error: Could not find an ellipse ...'
  digalog_warning, msg_txt
  q = DIALOG_MESSAGE(msg_txt,  /INFORMATION,  /CENTER, TITLE='choose the center')
  print, 'The TV got activated :)'
...
if strpos(dtype,'panstarrs') ge 0 and not file_test(aux_base+'_stv_roi_output.dat') and not file_test(aux_base+'_roi.dat') then begin
   ...
   msg_txt = 'Do you want to use SDSS masks (speed-up)?'
   digalog_warning, msg_txt
   g = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER)
   print, 'The TV got activated :)'
   if strcmp(g, 'Yes') then begin
      ...
      indx_lst = get_roi_sep_idl7(im_sdss, sdss_rfile)
      N = indx_lst[0]                   ; the number of arrays in the list 
      p = 1                             ; where the new array starts
      for j=0, N-1 do begin
            
            m = indx_lst[p]             ; the size of the array
            indx = indx_lst[p+1:p+m]    ; copying the corresponding array
            p = p+m+1                   ; changing to the beginning of the next array
...
; Grab the drawing focus in the given window
WSET, SLIDE_WINDOW

tv, dim,true=1 
endif else begin   ; if strue
 dim = im
 if indx[0] ge 0 then $
	 dim[indx] = smin
...
if strcmp(UVALUE, 'flag_gr2') then begin 
    if Event.value eq 0 then if Event.select then qa.disturbed = 1 else qa.disturbed = 0            
    if Event.value eq 1 then if Event.select then qa.trail = 1 else qa.trail = 0 
    if Event.value eq 2 then if Event.select then qa.not_spiral = 1 else qa.not_spiral = 0      
    if Event.value eq 3 then if Event.select then qa.face_on = 1 else qa.face_on = 0        
endif  


if strcmp(UVALUE, 'flag_gr3') then begin 
    if Event.value eq 0 then if Event.select then qa.faint = 1 else qa.faint = 0            
    if Event.value eq 1 then if Event.select then qa.crowded = 1 else qa.crowded = 0 
    if Event.value eq 2 then if Event.select then qa.over_masked = 1 else qa.over_masked = 0      
endif  
...
if strcmp(UVALUE, 'quality_gr') then begin 
    if Event.select then qa.quality = Event.value
endif 
...
common stvc, flag_w, pnale_w
   ...
   WIDGET_CONTROL, WIDGET_INFO(pnale_w, FIND_BY_UNAME='colBase31_widg'), sensitive=0
   ...
   flag_base =  WIDGET_BASE(TITLE='Flags & Note', uname='flag_base_uname', Row=9,  space=10, /BASE_ALIGN_CENTER, xoffset = xoff, yoffset = yoff)
      ...
      rowBase0 = widget_base(flag_base, COLUMN=2, /frame, space=20) 
        rowBase01 = widget_base(rowBase0, /col, /frame) 
         ...
         flag_label31 = WIDGET_Label(rowBase01, value = 'Back.radius : ' + strtrim(string(qa.backradius, FORMAT='(F5.2)'),2)+' arcmin', /ALIGN_LEFT)
      ...
      rowBase1 = widget_base(flag_base, /row, /frame) 
         flag_row1  = WIDGET_Label(rowBase1, value = 'FLAGS1:') 
      ...
      rowBase15 = widget_base(flag_base, /row, /frame) 
         flag_row15  = WIDGET_Label(rowBase15, value = 'FLAGS2:') 
         values = ['Disturbed (distorted)', 'Gasesous tail', 'NOT Spiral', 'Face-on'] 
         flag_gr2 = CW_BGROUP(rowBase15, values, UVALUE='flag_gr2', /Row, /NONEXCLUSIVE, $ 
               /FRAME, SET_VALUE=set_val, uname='flag_gr2_uname')       
         
      rowBase16 = widget_base(flag_base, /row, /frame) 
         flag_row16  = WIDGET_Label(rowBase16, value = 'FLAGS3:') 
         values = ['Low Signal (faint)', 'Crowded-field', 'Over-masked'] 
         flag_gr3 = CW_BGROUP(rowBase16, values, UVALUE='flag_gr3', /Row, /NONEXCLUSIVE, $ 
               /FRAME, SET_VALUE=set_val, uname='flag_gr3_uname')              
         
      rowBase2 = widget_base(flag_base, /row, /frame) 
         flag_row2  = WIDGET_Label(rowBase2, value = 'BAND1 [g,w1]:')  ; 'BAND1 [FUV,g,j]:'
      ...
      rowBase3 = widget_base(flag_base, /row, /frame) 
         flag_row3  = WIDGET_Label(rowBase3, value = 'BAND2 [r,w2]:') ; 'BAND2 [NUV,r h]:'
      ...
      rowBase4 = widget_base(flag_base, /row, /frame) 
         flag_row4  = WIDGET_Label(rowBase4, value = 'BAND3 [i,w3]:') ; 'BAND3 [i,k]    :'
         band3_gr = CW_BGROUP(rowBase4, values, UVALUE='band3_gr', /Row, /NONEXCLUSIVE, $ 
               /FRAME, SET_VALUE=set_val, uname='band3_gr_uname')
         
      rowBase6 = widget_base(flag_base, /row, /frame)    
         flag_row7  = WIDGET_Label(rowBase6, value = 'Photometry Quality < Bad ')
         bgroup2 = CW_BGROUP(rowBase6, ['0','1','2','3','4','5'], /Row, /EXCLUSIVE, /FRAME, SET_VALUE=qa.quality, UVALUE='quality_gr')
         flag_row7  = WIDGET_Label(rowBase6, value = ' Good >')
  ...
  XMANAGER, 'flag_base', flag_base, event_handler='flag_event', cleanup = 'flag_cleanup'
...
PRO stv_shutdown, status=status
...
if strcmp(misc, 'STOP') then begin
 ...
 msg_txt = 'Are you sure, you want to QUIT?'
 digalog_warning, msg_txt
 p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO, TITLE='Quite?')
 print, 'The TV got activated :)'
 if strcmp(p, 'No') then begin
    
    if keyword_set(status) and strcmp(status, 'User_mistake') then begin
        misc = 'GOTO'
        nlist[0] -= 1
        stv_shutdown, status='tmp'
        goto, finish
...
if strpos(misc,'STOP') lt 0  and $
   ...
   if (strpos(misc,'RUN') lt 0 and strpos(misc,'PREVIOUS') and strpos(misc,'SAVE') lt 0 and strpos(misc,'SKIP') lt 0) or strcmp(misc, 'QNEXT') then begin
	stv_print_qa_flags,/help,/values,/note,/details
...
if strcmp(misc, '') then begin
        p = ''
        msg_txt = 'Save your work !?'
        digalog_warning, msg_txt
        p = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, TITLE='Save?')
        digalog_warning, msg_txt
        if  strcmp(p, 'Yes') then Command_handler, 'save'
        misc = 'STOP'
        stv_shutdown, status='User_mistake'
endif
...
COMMON stvb, im, tim, sbase, draw, smin, smax, slog, sasinh, strue, ssmooth,$
   ...
   if strcmp(action, 'undo') then begin
      ...
      q = ''
      msg_txt = 'Use original ellipse info?'
      digalog_warning, msg_txt
      q = DIALOG_MESSAGE(msg_txt, /QUESTION,  /CENTER, /DEFAULT_NO, TITLE='recover the original ellipse?')
      print, 'The TV got activated :)'
   ...
   if strcmp(action, 'ell_catal') then begin   
      ...
      msg_txt = 'Use the input catalog ellipse info?'
      digalog_warning, msg_txt
...
PRO stv, image, jimg, usefile, object, width, height,  $
	 log=log, min=min, max=max, same=same, $
         ...
         ellipseinfo = ellipseinfo,  miscinfo = miscinfo, n_list=n_list, $
...
; Create a base
 ...
 wPrevious = Widget_Button( wFileMenu, VALUE='Goto ...', UVALUE='goto') 
 ...
 wELLsdss     = Widget_Button( wELLIPSE, VALUE='Common', UVALUE='ell_sdss')  
 ...
 clearBACK = Widget_Button( wBACK, VALUE='Clear Region', UVALUE='remove_back')  
 setBACK = Widget_Button( wBACK, VALUE='Modify back. radius "}"', UVALUE='set_back_radius')
 getBACK = Widget_Button( wBACK, VALUE='Get back. radius "{"', UVALUE='get_back_radius')
 resetBACK = Widget_Button( wBACK, VALUE='Reset back. radius ":"', UVALUE='reset_back_radius')
...
;     LABEL_TOP='Filter', /FRAME, SET_VALUE=set_val) 
  ...
  info_label2  = WIDGET_Label(info_label, value = 'Srv: '+STRUPCASE(dtype)) 
  
  info_label4  = WIDGET_Label(info_label, value = 'Obj: '+STRUPCASE(object))
  
  
  
  iloop = strtrim(string(n_list[0]+1),2)
  nloop = strtrim(string(n_list[1]),2)
  
  
  info_label3  = WIDGET_Label(info_label, value = '# '+iloop+'/'+nloop)
...
;   button7 = WIDGET_BUTTON(colBase00, VALUE='Edit/Add Centeroid (c)', UVALUE='c')
  ...
  star_ = widget_base(colBase00,COLUMN=3, /frame, uname='star_')
  star_1 = widget_base(star_, /column,/frame)
  star_11 = WIDGET_BUTTON(star_1, VALUE='-', UVALUE='star-') 
  star_2 = widget_base(star_, /column,/frame)
  star_22 = WIDGET_BUTTON(star_2, VALUE='star', UVALUE='starf')   
  star_3 = widget_base(star_, /column,/frame)
  star_33 = WIDGET_BUTTON(star_3, VALUE='+', UVALUE='star+')   
...
wLRow3 = widget_base(wLRow,COLUMN=2, /frame, uname='col3_widg') 
  ...
  colBase31 = widget_base(wLRow3, /column,/frame, uname='colBase31_widg')
  ...
  button161 = WIDGET_BUTTON(colBase31, VALUE='<REG>', UVALUE='import_reg')
  
  
wLRow4 = widget_base(wLRow,COLUMN=3, /frame, uname='col4_widg') 
  
  colBase4 = widget_base(wLRow4,/column, /frame) 
  button12 = WIDGET_BUTTON(colBase4, VALUE='Ellipse Size ..', UVALUE='none')	; UVALUE='e'
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Mouse pointer shape on the TV screen
;csel = 22                     ; Up arrow
;csel = 132                    ; Left arrow
;csel = 2                      ; Right arrow
;csel = 34                     ; X-hair.
;csel = 56                     ; Left hand.
;csel = 58                     ; Right hand.
csel = 33                      ; Narrow crosshair.
;  csel = 24                    ; Not-allowed
device, cursor_standard=csel
;;  device, /CURSOR_ORIGINAL     ; mouse pointer 
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
