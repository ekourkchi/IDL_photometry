pro stv_curval, hd, im, OFFSET = offset, ZOOM = zoom, Filename=Filename
;+
; NAME:
;       CURVAL
; PURPOSE:   
...
endcase
 ...
 if cr_err EQ 4 then begin
    print,' '
    if fileflag AND filename NE '/dev/tty' then free_lun,lun
    return
...
; Were left or center buttons been pressed?
 ...
 if (cr_err GE 1) and (cr_err LE 3) and (cr_err NE sv_err) then begin  
    ...
    if keyword_set(filename) and (not fileflag) then begin      ; open file & print table header to file
        get_lun,lun
        IF filename NE '/dev/tty' THEN openw,lun,filename,/append ELSE openw,lun,filename
    ...
    if keyword_set(filename) AND (filename NE '/dev/tty') then begin
        case curtype of 
