; $Id: twomass_radprof.pro,v 1.51 2014/03/03 19:23:35 neill Exp $
;+
; NAME:
;   
;   twomass_radprof
...
; PURPOSE:
;
;   generate radial profile of 2MASS data
;
; CATEGORY:
;
;   radial photometry
...
;    intfile: scalar string, 2MASS intensity map in mJy
;
...
; OUTPUTS:
;
;    total profile    = ID_BAND_totprofile.dat   BAND = j, h, or k
...
;
;   band='k'
;   filebase='~/ngc4656/data/2mass/fits/NGC4656B'
...
;   pa=38
;
;   resolve_routine,'twomass_radprof'
;
;   twomass_radprof,id,ra,dec,$
;                 majordiam,minordiam,pa,$
;                 intfile, $
...
;
ver=repstr('$Revision: 1.51 $ $Date: 2014/03/03 19:23:35 $','$','')
pre='TWOMASS_RADPROF: '
status = -1
...
if not int_fi.exists then begin 
 ...
 ;this change was made to accomodate large systematic errors in 2mass
 ;backgrounds and this may not be appropriate for other data (sdss, etc.)
 ss_annulus_int = annulus_int - annulus_skyint
 ss_int = total(ss_annulus_int>0.,/cumulative,/nan)
 ...
 printf,lun,'# TWOMASS_RADPROF '+ver+': total profile'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# TWOMASS_RADPROF '+ver+': annular profile'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# A_Gal:'+string(A_Galactic,form='(f7.3)')+' mag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# TWOMASS_RADPROF '+ver+': total magnitude'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# A_Gal:'+string(A_Galactic,form='(f7.3)')+' mag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# TWOMASS_RADPROF '+ver+': asymptotic magnitude'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# A_Gal:'+string(A_Galactic,form='(f7.3)')+' mag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# TWOMASS_RADPROF '+ver+': aperture magnitude'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# A_Gal:'+string(A_Galactic,form='(f7.3)')+' mag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# TWOMASS_RADPROF '+ver+': profile parameters'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# A_Gal:'+string(A_Galactic,form='(f7.3)')+' mag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# TWOMASS_RADPROF '+ver+': background'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# A_Gal:'+string(A_Galactic,form='(f7.3)')+' mag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# TWOMASS_RADPROF '+ver+': limiting mags'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# A_Gal:'+string(A_Galactic,form='(f7.3)')+' mag'
