pro rgb_panstarrs_asinh, fspec=fspec,scales=scales,brite=brite, $
	nonlinearity=nonlinearity, origin=origin, verbose=verbose, $
	quality=quality, outdir=outdir, update=update, $
	gonly=gonly,ronly=ronly,ionly=ionly,zonly=zonly
...
;-
; get file list
if not keyword_set(fspec) then fspec='*_z.fit*'
...
for i=0,n_elements(pfile)-1 do begin 
 ...
 gfile=repstr(pfile[i],'_z','_g')
 rfile=repstr(pfile[i],'_z','_r')
 ifile=repstr(pfile[i],'_z','_i')
 zfile=pfile[i]
 xfile=odir+'/'+(reverse(strsplit(pfile[i],'/',/extract)))[0]
 
 xfile=repstr(xfile,'_z','_gri')
