pro irac_plotcolors, id, pathtoprofile=pathtoprofile, type=type, $
    intfile=intfile, masklistfile=masklistfile, $
    jpgpath=jpgpath, dssfile=dssfile ,outpath=outpath
...
swbgfile=pathtoprofile+id+'_'+bands[b0]+'_'+'background.dat
readcol, swbgfile, swbg, swbg_e, swmu_bg, swmu_bg_e, swscale, $
         swskyradius_in, swskyradius_out,/silent

lwbgfile=pathtoprofile+id+'_'+bands[b1]+'_'+'background.dat
...
if gdpic then begin
 ...
 y1=ny0-delta > 0
 y2=ny0+delta < sz[1]-1
 gdimg=gdimg[x1:x2,y1:y2]
 sz=size(gdimg,/dim)
 scale=nscale[0]/60.0
 xoff=(x1+sz[0]/2) - (nx0+1.)
 yoff=(y1+sz[1]/2) - (ny0+1.)
 xrng = ([0-sz[0]/2,0+sz[0]/2]+xoff[0]) * scale
 yrng = ([0-sz[1]/2,0+sz[1]/2]+yoff[0]) * scale
 plotimage,gdimg,/preserve,charsize=chrsz,color=max(!d.n_colors),$
 imgxrange=xrng, imgyrange=yrng, ytitle='Arc Minutes',title='3.6um'
...
if rdpic then begin
 ...
 imgxrange=xrng, imgyrange=yrng, ytitle='Arc Minutes',title='4.5um'
...
if dssdata then begin
 ...
 if ~norband then uvscale=nscale else uvscale=gscale
 delta_dss = (delta*uvscale)/dssscale
 sz_dss=size(dss,/dim)
 ...
 scale=dssscale/60.0
 xoff=(x1_dss+sz[0]/2) - (dssx0+1.0)
 yoff=(y1_dss+sz[1]/2) - (dssy0+1.0)
 dxrng = ([0-sz[0]/2,0+sz[0]/2]+xoff[0]) * scale
 dyrng = ([0-sz[1]/2,0+sz[1]/2]+yoff[0]) * scale
 plotimage,dss,range=[min,max],/preserve,charsize=chrsz,$
  color=max(!d.n_colors),$
  imgxrange=dxrng, imgyrange=dyrng, ytitle='Arc Minutes',title='POSS2-R'
...
if xdpic then begin
 ...
 imgxrange=xrng, imgyrange=yrng, ytitle='Arc Minutes',title='Masked Composite'
