pro glga_batchprofile, lfile, $
        galex=galex,sdss=sdss,twomass=twomass,irac=irac,wise=wise, $
	verbose=verbose, annuli_size=annuli_size, logfile=logfile, $
	qa_run=qa_run
;+
; glga_batchprofile - run radial profile photometry on objects in list
;
; lfile - list of objects with these columns:
;       id	- string
;       ra,dec  - degrees
	...
	for j=0,nbands-1 do begin
		...
		if strpos(srvy,'galex') ge 0 then begin
			galex_radprof,id[i],r,d,$
				...
				fpath+id[i]+'_'+bands[j]+'_rr.fit*',$
				fpath+id[i]+'_'+bands[j]+'_cnt.fit*',$
