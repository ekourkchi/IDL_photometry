; $Id: wise_radprof.pro,v 1.46 2014/03/03 19:20:29 neill Exp $
;+
; NAME:
;   
;   wise_radprof
...
; PURPOSE:
;
;   generate radial profile of WISE data
;
; CATEGORY:
;
;   radial photometry
...
;    intfile: scalar string, WISE intensity map
;
...
;    background_radius: This is the semimajor-axis radius in arcmin, where the residual  
;                       background is calculated and then subtracted from the profile
; OUTPUTS:
;
...
;
;   band='w1'
;   filebase='~/ngc4656/data/wise/fits/NGC4656B'
...
;	2009-APR-09	D. Neill
;	2012-MAY-29	D. Neill - updated WISE noise model for drizzled images
;       2017-JUL-26     E. Kourkchi - introducing background_radius
;-
;
...
pro wise_radprof, $
    ...
    outpath=outpath, status=status, yuan13=yuan13, background_radius=background_radius
;
...
;
ver=repstr('$Revision: 1.46 $ $Date: 2014/03/03 19:20:29 $','$','')
pre='WISE_RADPROF: '
status = -1
...
; AB conversion from Table 5 in Section IV.3.g.v of 
; WISE Preliminary Release Explanatory Suppl.
...
;aboff=[2.683, 3.319, 5.242, 6.604]  (OBSOLETE)
 ...
 elist=file_search(rute+'_unc.fit*',count=nfe)
 
 bbb = strpos(intfile,'.fit')
 aaa = strpos(intfile,'_', /REVERSE_SEARCH)
 band_f = strmid(intfile,aaa+1,bbb-aaa-1) 
 
 if nfe eq 1 then begin
	; read variance image (sigma^2)
	var=( mrdfits(elist[0],0,varhdr,/fscale,/silent) )^2
 endif else begin
	var=-1.
 ...
 band=strmid(rute,strlen(rute)-2,2)

 if keyword_set(verbose) then print, pre+'processing '+band+' data'

 iband=(where(band EQ aband))[0]
 ...
 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ; convert images from DN to mJy
 magzp = sxpar(inthdr,'MAGZP') + aboff[iband]
 zpunc = sxpar(inthdr,'MAGZPUNC')>0.
 mjc = 10.^(0.4 * (16.4 - magzp))
 int = int * mjc
 var = var * mjc^2	; variance is in DN^2
 ...
 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ; get correlated pixel noise factor (variance)
 driz = 1.0
 ffact = 2.75
 if band eq 'w4' then ffact = ffact*2.0
 fcorr = (driz*ffact / as_pix)^2
 ...
 mmm,int[in]-skyint,skyresidual
 skyint=skyint+skyresidual
 ...
 ;; if sky background region has been set manually 
...
;  meanclip,my_sky,esn_sky,esn_skysig,clipsig=10
 mmm,my_sky,esn_sky,esn_skysig,nsky=nsky
 mmm,my_sky-esn_sky,skyresidual
 esn_sky=esn_sky+skyresidual
 ...
 Modback = 0
 if keyword_set(background_radius) then begin 
    if background_radius>0 then begin
        Modback = 1
    endif
 endif
...
;    skyint = 0.00215609
 ...
 ;err is uncertainty image (1-sigma)
 ...
 ;account for WISE correlated errors
 tot_int_var = tot_int_var * fcorr
 annulus_int_var = annulus_int_var * fcorr
 skyinterr = skyinterr * sqrt(fcorr)
 ...
 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 if Modback ge 1  and Modback le 1 then begin
 
;  best=where(a eq asymap)

best=where(a*as_pix/60. ge background_radius)
best = best[0]
 ...
 dPix = npix[best]-npix[best-2]
 
 dBack = (F2-F1)/dPix
 
 Back2 = tot_skyint[best]/npix[best]
 Back1 = tot_skyint[best-2]/npix[best-2]
 Back = 0.5*(Back1+Back2)
 
 
 print, 'dback:', dBack
 print, 'skyint:', skyint
 help, skyint
 skyint = (Back+dBack)*skyint/Back
 skyinterr = (Back+dBack)*skyinterr/Back
 
 Modback += 1
 skyint = skyint[0]
 back_modify_percent = 100.*(dBack)/Back
 print, 'modification percent:', back_modify_percent
 print , 'skyint:', skyint
 help, skyint
 ...
 printf,lun,'# WISE_RADPROF '+ver+': total profile'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# MagZp:'+string(magzp,form='(f7.3)')+' ABmag'
 printf,lun,'# ZpUnc:'+string(zpunc,form='(f7.3)')+' mag'
 printf,lun,'# DNmJy:'+string(mjc,form='(g9.3)')+' mJy/DN'
 ...
 printf,lun,'# Intensity map  (DN): ',int_fi.name,$
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# WISE_RADPROF '+ver+': annular profile'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# MagZp:'+string(magzp,form='(f7.3)')+' ABmag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# WISE_RADPROF '+ver+': total magnitude'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# MagZp:'+string(magzp,form='(f7.3)')+' ABmag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# WISE_RADPROF '+ver+': asymptotic magnitude'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# MagZp:'+string(magzp,form='(f7.3)')+' ABmag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# WISE_RADPROF '+ver+': aperture magnitude'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# MagZp:'+string(magzp,form='(f7.3)')+' ABmag'
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# WISE_RADPROF '+ver+': ellipse parameters'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# MagZp:'+string(magzp,form='(f7.3)')+' ABmag'
 ...
 v=['bg','bg_e','mu_bg', 'mu_bg_e', 'scale','skyradius_in','skyradius_out', 'back_modify_radius', 'back_modify_percent']
 u=['[mJy/as^2]','[mJy/as^2]','[mag/as^2]','[mag/as^2]','[as/pixel]','[as]','[as]', '[as]', '[%]']

 if not keyword_set(outpath) then outpath="./"
 outfile=outpath+'/'+id+'_'+band+'_background.dat' 
 openw,lun,outfile,/get_lun
 printf,lun,'# WISE_RADPROF '+ver+': background'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# MagZp:'+string(magzp,form='(f7.3)')+' ABmag'
 ...
 if keyword_set(maskimgfile) then  $
  ...
  format='(a,a'+strcompress(strlen(maskimgfile))+')'
 printf,lun,'#'
 printf,lun,'# back_modify_radius : where to calculate the background residual for further modifications'
 printf,lun,'# back_modify_percent: the percentage of the original background level added for modification'
 printf,lun,'# Additional info in ellipse parameter file and background file.'
 printf,lun,'#'
 printf,lun,'#',v[0],v[1],v[2],v[3],v[4],v[5],v[6],v[7],v[8], $
   format='(a1,a11,x,a12,x,a12,x,a12,x,a12,x,a14,x,a14,a20,a20)'
 printf,lun,'#',u[0],u[1],u[2],u[3],u[4],u[5],u[6],u[7],u[8], $
   format='(a1,a11,x,a12,x,a12,x,a12,x,a12,x,a14,x,a14,a20,a20)'
 array_clean,-99.999,0.,50.,musky,muskyerr,/nan
 printf,lun, skyint/as_pix^2, skyinterr/as_pix^2, musky, muskyerr, as_pix,$
              skyinner*as_pix, skyouter*as_pix,background_radius*60.,back_modify_percent, $
 format='(e12.5,x,e12.5,x,f12.3,x,f12.3,x,f12.3,x,f14.3,x,f14.3,f20.3,f20.3)'
 free_lun,lun
 ...
 openw,lun,outfile,/get_lun
 printf,lun,'# WISE_RADPROF '+ver+': limiting mags'
 printf,lun,'# Date : '+systime()
 printf,lun,'# ID   : '+id
 printf,lun,'# Band : '+band
 printf,lun,'# MagZp:'+string(magzp,form='(f7.3)')+' ABmag'
