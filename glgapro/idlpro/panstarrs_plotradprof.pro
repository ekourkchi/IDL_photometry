pro panstarrs_plotradprof, id, pathtoprofile=pathtoprofile, type=type, $
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 if keyword_set(jpgpath) and keyword_set(auxbase) then begin
   ...
   rute = strmid(rute,0,strpos(rute,'/panstarrs'))
   mask_g = rute+'/aux/'+id+'_panstarrs_mask_g.fits.gz'
   mask_r = rute+'/aux/'+id+'_panstarrs_mask_r.fits.gz'
   mask_i = rute+'/aux/'+id+'_panstarrs_mask_i.fits.gz'
   mask_z = rute+'/aux/'+id+'_panstarrs_mask_z.fits.gz'
...
filename=outpath+'/'+id+'_panstarrs_profile.ps'
...
nminmax=minmax([gtot_mag,rtot_mag,itot_mag,ztot_mag], /nan)
min=nminmax[0]
max=nminmax[1]
...
nminmax=minmax([gann_mu,rann_mu,iann_mu,zann_mu], /nan)
...
legend,bands[[b1,b2,b3,b4]],textcolors=cs[[b1,b2,b3,b4]],/bot,/left, $
	box=0, charsize=1.0
...
filename=outpath+'/'+id+'_panstarrs_images.ps'
ms_ps_start, filename=filename, xsize=10.5,ysize=8.,/inch,$
...
filename=outpath+'/'+id+'_panstarrs_images_masks.ps'
ms_ps_start, filename=filename, xsize=10.5,ysize=8.,/inch,$
          /color,bit=8, xoffset=0.15, yoffset = 11,/landscape
...
if gdpic then begin
 ...
 if g_bmap_avail then begin 
    col = (rx0-g_col)*scale
    row = (g_row-ry0)*scale
    indx_tmp = where(col gt xpltrng[1] and col lt xpltrng[0])
    col = col[indx_tmp]
    row = row[indx_tmp]
    indx_tmp = where(row gt ypltrng[0] and row lt ypltrng[1])
    row = row[indx_tmp]
    col = col[indx_tmp] 
    
    plots, col, row, color=!red, psym=3
 endif
...
p=outpath+'/'
name=id+'_panstarrs_images_masks'
spawn,'convert +matte -density 196 -resample 72 -rotate -90 '+p+name+'.ps '+p+name+'.jpg'
spawn,'ps2pdf14 '+p+name+'.ps '+p+name+'.pdf'
spawn,'rm '+p+name+'.ps'
