...
def split_func(sex_file):
    ...
    output = Popen(
        ["awk", "(NR>2){print($6\" \"$8\" \"$16\" \"$18\" \"$22)}", sex_file], stdout=PIPE)
...
def main():
    ...
    if gri_image is not None:
        ...
        imgplot = plt.imshow(image_data, cmap='gray',
                             vmin=scale_min, vmax=scale_max, norm=LogNorm())
        axis([x_min, x_max, y_min, y_max])
    else:
    ...
    def help_me(event):
        ...
        if os.path.isfile(help_fi):
            subprocess.Popen('python ' + py_root +
                             'help_widget.py  -x -f '+help_fi+' &', shell=True)
    ...
    def save(event):
        ...
        ellipse_string = '{:.2f}'.format(2*a*0.396) + '  ' + '{:.2f}'.format(2*b*0.396) + '  ' + '{:.5f}'.format(RA) + '  ' + '{:.5f}'.format(DEC) + '  ' + '{:.2f}'.format(
            pa-90)+'  ' + '{:.2f}'.format(a)+'  '+'{:.2f}'.format(b) + '  ' + '{:.2f}'.format(xcenter)+'  '+'{:.2f}'.format(ycenter)+'  1  0.396'
